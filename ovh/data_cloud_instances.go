	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"instances": schema.ListNestedAttribute{
				Computed:            true,
				Description:         "Instances of the project",
				MarkdownDescription: "Instances of the project",
				NestedObject: schema.NestedAttributeObject{Attributes: map[string]schema.Attribute{
					"id":                schema.StringAttribute{CustomType: ovhtypes.TfStringType{}, Computed: true, Description: instanceDescId, MarkdownDescription: instanceDescId},
...
func buildInstanceListItemObject(ctx context.Context, response *CloudInstanceAPIResponse) basetypes.ObjectValue {
	...
	if response.TargetSpec != nil {
		...
		nameVal = tfStr(ts.Name)
		if ts.Location != nil {
			regionVal = tfStr(ts.Location.Region)
			if ts.Location.AvailabilityZone != "" {
				azVal = tfStr(ts.Location.AvailabilityZone)
			}
		}
		if ts.Flavor != nil {
			flavorVal = tfStr(ts.Flavor.Id)
		}
		if ts.Image != nil && ts.Image.Id != "" {
			imageVal = tfStr(ts.Image.Id)
		}
		if ts.PowerState != "" {
			powerVal = tfStr(ts.PowerState)
		}
	}
	...
	obj, _ := types.ObjectValue(InstanceListItemAttrTypes(), map[string]attr.Value{
		...
		"checksum":          tfStr(response.Checksum),
		"created_at":        tfStr(response.CreatedAt),
		"updated_at":        tfStr(response.UpdatedAt),
		"resource_status":   tfStr(response.ResourceStatus),
