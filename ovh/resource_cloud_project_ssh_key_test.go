...
// A public key read with file() keeps the trailing newline of the .pub file.
// The API stores the key trimmed, which must not make the apply fail with
// "Provider produced inconsistent result after apply" nor produce a diff on
// the next plan.
func TestAccCloudProjectSSHKey_trailingNewline(t *testing.T) {
	keyName := acctest.RandomWithPrefix(test_prefix)
	publicKey := "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAgQC9xPpdqP3sx2H+gcBm65tJEaUbuifQ1uGkgrWtNY0PRKNNPdy+3yoVOtxk6Vjo4YZ0EU/JhmQfnrK7X7Q5vhqYxmozi0LiTRt0BxgqHJ+4hWTWMIOgr+C2jLx7ZsCReRk+fy5AHr6h0PHQEuXVLXeUy/TDyuY2JPtUZ5jcqvLYgQ== my-nuclear-power-plant"
	config := fmt.Sprintf(`
	resource "ovh_cloud_project_ssh_key" "key" {
		service_name = "%s"
		region       = "GRA9"
	...
	`, os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST"), keyName, publicKey)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ovh_cloud_project_ssh_key.key", "name", keyName),
					resource.TestCheckResourceAttr("ovh_cloud_project_ssh_key.key", "public_key", publicKey+"\n"),
				),
			},
		},
	})
