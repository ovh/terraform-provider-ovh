...
func TestAccCloudManagedDatabaseIntegration_importBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:      "ovh_cloud_managed_database_integration.integration",
				ImportState:       true,
				ImportStateVerify: true,
				ImportStateIdFunc: testAccCloudManagedDatabaseIntegrationImportId("ovh_cloud_managed_database_integration.integration"),
			},
		},
	})
}

func testAccCloudManagedDatabaseIntegrationImportId(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		testIntegration, ok := s.RootModule().Resources[resourceName]
		if !ok {
			return "", fmt.Errorf("ovh_cloud_managed_database_integration not found: %s", resourceName)
		}
		return fmt.Sprintf(
			"%s/%s/%s/%s",
