...
func TestAccCloudManagedDatabaseMongodbUser_importBasic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				ResourceName:            "ovh_cloud_managed_database_mongodb_user.user",
				ImportState:             true,
				ImportStateVerify:       true,
				ImportStateIdFunc:       testAccCloudManagedDatabaseMongodbUserImportId("ovh_cloud_managed_database_mongodb_user.user"),
...
func testAccCloudManagedDatabaseMongodbUserImportId(resourceName string) resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		testMongodbUser, ok := s.RootModule().Resources[resourceName]
		if !ok {
			return "", fmt.Errorf("ovh_cloud_managed_database_mongodb_user not found: %s", resourceName)
		}
		return fmt.Sprintf(
			"%s/%s/%s",
