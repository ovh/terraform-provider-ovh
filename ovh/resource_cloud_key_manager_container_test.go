...
func TestAccCloudKeyManagerContainer_rsa(t *testing.T) {
	serviceName := os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST")
	region := os.Getenv("OVH_CLOUD_PROJECT_REGION_TEST")
	privateName := acctest.RandomWithPrefix(test_prefix)
	publicName := acctest.RandomWithPrefix(test_prefix)
	containerName := acctest.RandomWithPrefix(test_prefix)

	// An RSA container references a PRIVATE and a PUBLIC secret.
	config := fmt.Sprintf(`
resource "ovh_cloud_key_manager_secret" "private" {
  service_name = "%s"
  region       = "%s"
  name         = "%s"
  secret_type  = "PRIVATE"
...
resource "ovh_cloud_key_manager_secret" "public" {
  service_name = "%s"
  region       = "%s"
  name         = "%s"
  secret_type  = "PUBLIC"
...
resource "ovh_cloud_key_manager_container" "test" {
  ...
  secret_refs = [
    {
      name      = "private_key"
      secret_id = ovh_cloud_key_manager_secret.private.id
    },
    {
      name      = "public_key"
      secret_id = ovh_cloud_key_manager_secret.public.id
    }
  ]
}
`, serviceName, region, privateName, serviceName, region, publicName, serviceName, region, containerName)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "name", containerName),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "type", "RSA"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "secret_refs.#", "2"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_container.test", "id"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_container.test", "checksum"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "resource_status", "READY"),
...
func TestAccCloudKeyManagerContainer_removeSecretRefs(t *testing.T) {
	serviceName := os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST")
	region := os.Getenv("OVH_CLOUD_PROJECT_REGION_TEST")
	secretName := acctest.RandomWithPrefix(test_prefix)
	containerName := acctest.RandomWithPrefix(test_prefix)
...
`, serviceName, region, secretName, serviceName, region, containerName)
	...
	// Same container, secret_refs block removed entirely.
	updatedConfig := fmt.Sprintf(`
resource "ovh_cloud_key_manager_secret" "secret1" {
  service_name = "%s"
  region       = "%s"
...
`, serviceName, region, secretName, serviceName, region, containerName)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Remove all secret_refs: the container must update in place and
			// return to READY once the OUT_OF_SYNC re-sync flow completes.
			{
				Config: updatedConfig,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "secret_refs.#", "0"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_container.test", "checksum"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_container.test", "resource_status", "READY"),
				),
			},
