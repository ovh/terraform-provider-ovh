...
func TestAccCloudSshKeyDataSource_basic(t *testing.T) {
	...
	data "ovh_cloud_ssh_key" "key" {
		service_name = ovh_cloud_ssh_key.key.service_name
		name         = ovh_cloud_ssh_key.key.name
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.ovh_cloud_ssh_key.key", "name", keyName),
					resource.TestCheckResourceAttrSet("data.ovh_cloud_ssh_key.key", "public_key"),
					resource.TestCheckResourceAttrSet("data.ovh_cloud_ssh_key.key", "created_at"),
					resource.TestCheckResourceAttrSet("data.ovh_cloud_ssh_key.key", "updated_at"),
				),
			},
		},
	})
