...
func CloudProjectRancherDataSourceSchema(ctx context.Context) schema.Schema {
	...
	return schema.Schema{
		Description: "Get a managed Rancher service",
		Attributes:  attrs,
	}
}
...
func NewCurrentStateValue(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) (CurrentStateValue, diag.Diagnostics) {
	...
	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`ip_restrictions expected to be basetypes.ListValue, was: %T`, ipRestrictionsAttribute))
	}
...
func (v *CurrentStateValue) MergeWith(other *CurrentStateValue) {
	...
	if (v.Networking.IsUnknown() || v.Networking.IsNull()) && !other.Networking.IsUnknown() {
	...
	if (v.Usage.IsUnknown() || v.Usage.IsNull()) && !other.Usage.IsUnknown() {
		v.Usage = other.Usage
	}
