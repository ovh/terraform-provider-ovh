...
func lbIntPtr(v int) *int { return &v }
...
func (m *CloudLoadbalancerListenerModel) ToCreate() *CloudLoadbalancerListenerCreatePayload {
	...
	if !m.ConnectionLimit.IsNull() && !m.ConnectionLimit.IsUnknown() {
		targetSpec.ConnectionLimit = lbIntPtr(int(m.ConnectionLimit.ValueInt64()))
	}

	if !m.TimeoutClientData.IsNull() && !m.TimeoutClientData.IsUnknown() {
		targetSpec.TimeoutClientData = lbIntPtr(int(m.TimeoutClientData.ValueInt64()))
	}

	if !m.TimeoutMemberData.IsNull() && !m.TimeoutMemberData.IsUnknown() {
		targetSpec.TimeoutMemberData = lbIntPtr(int(m.TimeoutMemberData.ValueInt64()))
	}

	if !m.TimeoutMemberConnect.IsNull() && !m.TimeoutMemberConnect.IsUnknown() {
		targetSpec.TimeoutMemberConnect = lbIntPtr(int(m.TimeoutMemberConnect.ValueInt64()))
	}

	if !m.TimeoutTcpInspect.IsNull() && !m.TimeoutTcpInspect.IsUnknown() {
		targetSpec.TimeoutTcpInspect = lbIntPtr(int(m.TimeoutTcpInspect.ValueInt64()))
	}
