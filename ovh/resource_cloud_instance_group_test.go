...
// changing the name (a RequiresReplace attribute) forces a replace. Replacement
// is also the remediation path for OUT_OF_SYNC drift
// (RequiresReplaceIfOutOfSyncPlanModifier on resource_status), which cannot be
// provoked from a test since it requires an out-of-band change.
