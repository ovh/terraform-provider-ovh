...
func TestAccCloudKeyManagerSecret_symmetric(t *testing.T) {
	serviceName := os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST")
	region := os.Getenv("OVH_CLOUD_PROJECT_REGION_TEST")
	name := acctest.RandomWithPrefix(test_prefix)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "region", region),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "name", name),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "secret_type", "SYMMETRIC"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "algorithm", "AES"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "bit_length", "256"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "mode", "CBC"),
					// Algorithm/bit_length/mode are reported back by OpenStack Barbican
					// in current_state; assert they are populated.
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "current_state.algorithm"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "current_state.bit_length"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "current_state.mode"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "id"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "checksum"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "resource_status", "READY"),
				),
...
func TestAccCloudKeyManagerSecret_withExpiration(t *testing.T) {
	serviceName := os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST")
	region := os.Getenv("OVH_CLOUD_PROJECT_REGION_TEST")
	name := acctest.RandomWithPrefix(test_prefix)
...
`, serviceName, region, name, expiration)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "name", name),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "secret_type", "OPAQUE"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "expiration", expiration),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "id"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "resource_status", "READY"),
				),
			},
...
func TestAccCloudKeyManagerSecret_certificate(t *testing.T) {
	serviceName := os.Getenv("OVH_CLOUD_PROJECT_SERVICE_TEST")
	region := os.Getenv("OVH_CLOUD_PROJECT_REGION_TEST")
	name := acctest.RandomWithPrefix(test_prefix)

	// CERTIFICATE secret with no payload.
	config := fmt.Sprintf(`
resource "ovh_cloud_key_manager_secret" "test" {
  service_name = "%s"
  region       = "%s"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "name", name),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "secret_type", "CERTIFICATE"),
					resource.TestCheckResourceAttrSet("ovh_cloud_key_manager_secret.test", "id"),
					resource.TestCheckResourceAttr("ovh_cloud_key_manager_secret.test", "resource_status", "READY"),
				),
			},
