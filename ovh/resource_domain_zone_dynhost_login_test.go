...
func testSweepDomainZoneDynhostLogin(region string) error {
	...
	if err := client.Get(fmt.Sprintf("/domain/zone/%s/dynhost/login", zoneName), &logins); err != nil {
	...
	for _, l := range logins {
		...
		if err := client.Get(fmt.Sprintf("/domain/zone/%s/dynhost/login/%s", zoneName, l), &login); err != nil {
