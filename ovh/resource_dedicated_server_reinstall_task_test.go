...
func TestAccDedicatedServerReinstall_byo(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAccPreCheckCredentials(t)
			testAccPreCheckDedicatedServer(t)
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDedicatedServerReinstallConfig("byo"),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr(
						"ovh_dedicated_server_update.server", "state", "ok"),
					resource.TestCheckResourceAttr(
...
func testAccDedicatedServerReinstallConfig(config string) string {
	...
	if config == "byo" {
		return fmt.Sprintf(
			testAccDedicatedServerReinstallConfig_Byo,
			dedicated_server,
		)
	}
...
`
const testAccDedicatedServerReinstallConfig_Byo = `
data ovh_dedicated_server_boots "harddisk" {
  service_name = "%s"
  boot_type    = "harddisk"
}
