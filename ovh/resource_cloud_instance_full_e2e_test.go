		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					// Two NICs: gateway egress and a floating IP on the private one, an
					// auto-assigned Ext-Net address on the public one.
					resource.TestCheckResourceAttr(rn, "networks.#", "2"),
					resource.TestCheckResourceAttr(rn, "current_state.networks.#", "2"),
					resource.TestCheckResourceAttrSet(rn, "current_state.networks.0.addresses.#"),
					resource.TestCheckResourceAttrSet(rn, "current_state.networks.1.addresses.#"),
