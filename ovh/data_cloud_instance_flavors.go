...
// CloudInstanceFlavorsModel is the model for the plural flavors data source.
...
func (d *cloudInstanceFlavorsDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description:         "Use this data source to list the flavors available in a public cloud project. This is read-only reference data: each entry describes a hardware sizing (vCPUs, RAM, disks) the project can create instances from, not an existing resource.",
		MarkdownDescription: "Use this data source to list the flavors available in a public cloud project. This is read-only reference data: each entry describes a hardware sizing (vCPUs, RAM, disks) the project can create instances from, not an existing resource.",
		Attributes: map[string]schema.Attribute{
			...
			"region": schema.StringAttribute{
				CustomType:          ovhtypes.TfStringType{},
				Optional:            true,
				Description:         "Restrict the listing to the flavors offered in this region. The catalog is per-region: a flavor returned for one region is not guaranteed to exist, or to carry the same characteristics, in another.",
				MarkdownDescription: "Restrict the listing to the flavors offered in this region. The catalog is per-region: a flavor returned for one region is not guaranteed to exist, or to carry the same characteristics, in another.",
			},
			"flavors": schema.ListNestedAttribute{
				Computed:            true,
				Description:         "Flavors advertised by the backend for this project, one entry per region the flavor is offered in.",
				MarkdownDescription: "Flavors advertised by the backend for this project, one entry per region the flavor is offered in.",
				NestedObject: schema.NestedAttributeObject{
					Attributes: instanceFlavorDataSourceAttributes(),
				},
			},
		},
	}
...
func (d *cloudInstanceFlavorsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	itemObjType := types.ObjectType{AttrTypes: instanceFlavorItemAttrTypes()}
	items := make([]attr.Value, 0, len(responseData))
	for i := range responseData {
		var m CloudInstanceFlavorModel
		m.MergeWith(ctx, &responseData[i])
		obj, diags := types.ObjectValue(instanceFlavorItemAttrTypes(), map[string]attr.Value{
			"id":          m.Id,
			"name":        m.Name,
			"vcpus":       m.Vcpus,
			"ram":         m.Ram,
