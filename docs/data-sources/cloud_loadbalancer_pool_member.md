...
# ovh_cloud_loadbalancer_pool_member (Data Source)
...
```terraform
data "ovh_cloud_loadbalancer_pool_member" "member" {
...
output "member_status" {
  value = data.ovh_cloud_loadbalancer_pool_member.member.current_state.operating_status
}
```
...
* `current_state` - Current state of the member:
  ...
  * `monitor` - Health monitor address and port override (same schema as `monitor`).
