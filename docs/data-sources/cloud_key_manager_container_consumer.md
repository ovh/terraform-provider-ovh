...
# ovh_cloud_key_manager_container_consumer (Data Source)
...
* `consumer_id` - (Required) Consumer identifier, as returned in the `id` attribute of the `ovh_cloud_key_manager_container_consumer` resource and the `id` field of the `ovh_cloud_key_manager_container_consumers` data source (the base64 identifier expected by the get-one endpoint).
