...
# ovh_cloud_loadbalancer_pool (Data Source)
...
```terraform
data "ovh_cloud_loadbalancer_pool" "pool" {
...
output "pool_algorithm" {
  value = data.ovh_cloud_loadbalancer_pool.pool.algorithm
}
```
...
* `health_monitor` - Health monitor configuration:
  ...
  * `max_retries` - Number of consecutive health check failures before marking a member as unhealthy (1-10).
  * `max_retries_down` - Number of consecutive health check failures before marking a member as `ERROR` (1-10).
  * `name` - Health monitor name.
  * `url_path` - URL path for HTTP/HTTPS health checks.
  * `http_method` - HTTP method for health checks (`GET`, `HEAD`, `POST`, `PUT`, `DELETE`, `PATCH`, `OPTIONS`, `TRACE`).
  * `http_version` - HTTP version for health checks (`1.0` or `1.1`).
  * `expected_codes` - Expected HTTP response codes (e.g. `200`, `200-202`).
...
* `current_state` - Current state of the pool:
  ...
  * `persistence` - Session persistence configuration (same schema as `persistence`).
  * `health_monitor` - Health monitor configuration (same schema as `health_monitor`), plus:
