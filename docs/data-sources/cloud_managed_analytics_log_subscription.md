...
# ovh_cloud_managed_analytics_log_subscription (Data Source)
...
```terraform
data "ovh_cloud_managed_analytics_log_subscription" "subscription" {
    service_name = "VVV"
    engine       = "XXX"
    cluster_id   = "YYY"
    id           = "ZZZ"
...
output "subscription_ldp_name" {
  value = data.ovh_cloud_managed_analytics_log_subscription.subscription.ldp_service_name
}
```
