...
# ovh_cloud_managed_database_ip_restrictions (Data Source)

~> **DEPRECATED** Use `ip_restrictions` field in `ovh_cloud_managed_database` resource instead.
...
```terraform
data "ovh_cloud_managed_database_ip_restrictions" "ip_restrictions" {
  service_name  = "XXXXXX"
  engine        = "YYYY"
  cluster_id    = "ZZZZ"
}
...
output "ips" {
  value = data.ovh_cloud_managed_database_ip_restrictions.ip_restrictions.ips
}
```
