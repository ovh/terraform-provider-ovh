...
# ovh_vps_status (Data Source)

Use this data source to retrieve the network/service probe status of a VPS
(ping, dns, http, https, ssh, smtp, tools).
...
* `probes` - List of service probes:
  * `service` - Service name (ping, dns, http, https, ssh, smtp, tools).
  * `port` - TCP/UDP port (when applicable).
  * `state` - Probe state (typically `up` or `down`).
