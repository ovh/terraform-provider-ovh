...
# ovh_cloud_loadbalancer_listener (Data Source)
...
```terraform
data "ovh_cloud_loadbalancer_listener" "http" {
...
output "listener_port" {
  value = data.ovh_cloud_loadbalancer_listener.http.protocol_port
}
```
...
* `id` - (Required) ID of the listener.
...
* `protocol` - Protocol of the listener (`HTTP`, `HTTPS`, `SCTP`, `TCP`, `TERMINATED_HTTPS`, `UDP`).
...
* `current_state` - Current state of the listener:
  ...
  * `insert_headers` - Headers inserted into requests (same schema as `insert_headers`).
  * `allowed_cidrs` - List of CIDRs allowed to access the listener.
  * `sni_container_refs` - List of SNI container references.
  * `tls_versions` - List of TLS versions allowed.
