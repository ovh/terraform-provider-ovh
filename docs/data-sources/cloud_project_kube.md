...
* `customization_cilium` - Cilium CNI customization.
  * `cluster_id` - Cilium cluster ID, between 1 and 255.
  * `cluster_mesh` - ClusterMesh feature configuration.
    * `enabled` - Whether the ClusterMesh feature is enabled.
    * `api_server` - ClusterMesh API server exposure configuration.
      * `node_port` - NodePort on which the ClusterMesh API server is exposed.
      * `service_type` - Service type used to expose the ClusterMesh API server.
  * `hubble` - Hubble observability stack configuration.
    * `enabled` - Whether Hubble is enabled.
    * `relay` - Hubble Relay configuration.
      * `enabled` - Whether Hubble Relay is enabled.
    * `ui` - Hubble UI configuration.
      * `enabled` - Whether the Hubble UI is enabled.
      * `backend_resources` - Resource configuration for the Hubble UI backend.
      ...
      * `frontend_resources` - Resource configuration for the Hubble UI frontend.
        * `limits` - Resource limits.
          * `cpu` - CPU limit.
          * `memory` - Memory limit.
        * `requests` - Resource requests.
          * `cpu` - CPU request.
          * `memory` - Memory request.
* `ip_allocation_policy` - IP allocation policy of the cluster.
  * `pods_ipv4_cidr` - CIDR used for the cluster's pods.
  * `services_ipv4_cidr` - CIDR used for the cluster's services.
