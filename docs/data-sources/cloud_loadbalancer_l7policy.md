...
# ovh_cloud_loadbalancer_l7policy (Data Source)
...
```terraform
data "ovh_cloud_loadbalancer_l7policy" "policy" {
...
output "l7policy_action" {
  value = data.ovh_cloud_loadbalancer_l7policy.policy.action
}
```
...
* `id` - (Required) ID of the L7 policy.
