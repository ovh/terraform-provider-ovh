...
- `deletion_protection` - (Optional) Prevent the cloud project from being destroyed. When set to `true`, `terraform destroy` will fail with an error. You must set this to `false` and run `terraform apply` before the resource can be destroyed. Defaults to `false`.
