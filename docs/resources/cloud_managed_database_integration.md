...
# ovh_cloud_managed_database_integration
...
```terraform
data "ovh_cloud_managed_database" "db_postgresql" {
  service_name  = "XXXX"
  engine        = "postgresql"
  id            = "ZZZZ"
}

data "ovh_cloud_managed_database" "db_opensearch" {
  service_name  = "XXXX"
  engine        = "opensearch"
  id            = "ZZZZ"
}
...
resource "ovh_cloud_managed_database_integration" "integration" {
  service_name            = data.ovh_cloud_managed_database.db_postgresql.service_name
  engine                  = data.ovh_cloud_managed_database.db_postgresql.engine
  cluster_id              = data.ovh_cloud_managed_database.db_postgresql.id
  source_service_id       = data.ovh_cloud_managed_database.db_postgresql.id
  destination_service_id  = data.ovh_cloud_managed_database.db_opensearch.id
  type                    = "opensearchLogs"
}
```
...
```bash
$ terraform import ovh_cloud_managed_database_integration.my_integration service_name/engine/cluster_id/id
```
