...
# ovh_cloud_managed_analytics_m3db_user
...
output "user_password" {
  value     = ovh_cloud_managed_analytics_m3db_user.user.password
  sensitive = true
}
```
...
resource "ovh_cloud_managed_analytics_m3db_user" "user_datetime" {
  service_name    = data.ovh_cloud_managed_analytics.m3db.service_name
  cluster_id      = data.ovh_cloud_managed_analytics.m3db.id
  group           = "mygroup"
  name            = "alice"
...
resource "ovh_cloud_managed_analytics_m3db_user" "user_md5" {
  service_name    = data.ovh_cloud_managed_analytics.m3db.service_name
  cluster_id      = data.ovh_cloud_managed_analytics.m3db.id
  group           = "mygroup"
  name            = "bob"
...
```bash
$ terraform import ovh_cloud_managed_analytics_m3db_user.my_user service_name/cluster_id/id
```
