...
# ovh_cloud_managed_database_postgresql_user
...
```terraform
data "ovh_cloud_managed_database" "postgresql" {
  service_name  = "XXXX"
  engine        = "postgresql"
  id            = "ZZZZ"
}
...
resource "ovh_cloud_managed_database_postgresql_user" "user" {
  service_name  = data.ovh_cloud_managed_database.postgresql.service_name
  cluster_id    = data.ovh_cloud_managed_database.postgresql.id
  name          = "johndoe"
  roles         = ["replication"]
}

output "user_password" {
  value     = ovh_cloud_managed_database_postgresql_user.user.password
  sensitive = true
}
```
...
resource "ovh_cloud_managed_database_postgresql_user" "user_datetime" {
  service_name    = data.ovh_cloud_managed_database.postgresql.service_name
  cluster_id      = data.ovh_cloud_managed_database.postgresql.id
  name            = "alice"
  roles           = ["replication"]
...
resource "ovh_cloud_managed_database_postgresql_user" "user_md5" {
  service_name    = data.ovh_cloud_managed_database.postgresql.service_name
  cluster_id      = data.ovh_cloud_managed_database.postgresql.id
  name            = "bob"
  roles           = ["replication"]
...
```bash
$ terraform import ovh_cloud_managed_database_postgresql_user.my_user service_name/cluster_id/id
```
