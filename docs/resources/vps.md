...
* `id` - Service name of the VPS (e.g., "vps-123456.vps.ovh.net"); identical to `service_name`
* `service_name` - Service name of the VPS
* `iam` - IAM resource information
  * `urn` - URN of the VPS, used when writing IAM policies
...
* `cluster` - VPS cluster
* `display_name` - Custom display name
* `do_not_send_password` - Whether the default password creation email was skipped at install time
* `keymap` - KVM keyboard layout on VPS Cloud
* `memory_limit` - RAM of this VPS (in MB)
* `model` - Structure describing characteristics of a VPS model
  ...
  * `version` - All versions that VPS can have (2013v1┃2014v1┃2015v1┃2017v1┃2017v2┃2017v3┃2018v1┃2018v2┃2019v1┃2025v1)
...
* `sla_monitoring` - Whether OVHcloud SLA monitoring is enabled on this VPS
* `state` - State of the VPS (backuping┃installing┃maintenance┃rebooting┃rescued┃running┃stopped┃stopping┃upgrading)
* `vcore` - Number of vcores
* `zone` - OpenStack region where the VPS is located

## Import

OVHcloud VPS can be imported using the `service_name`, E.g.,
