...
Creates an user for a Clickhouse cluster associated with a public cloud project.

## Example Usage

Create a user johndoe in a Clickhouse database. Output the user generated password with command `terraform output user_password`.
...
```bash
$ terraform import ovh_cloud_project_database_clickhouse_user.my_user service_name/cluster_id/id
```