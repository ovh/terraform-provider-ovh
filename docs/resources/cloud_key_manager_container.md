...
# ovh_cloud_key_manager_container
...
* `availability_zone` - (Optional, Forces new resource) Availability zone where the container will be created.
...
* `secret_refs` - (Optional) List of secret references in the container. Each element supports:
...
```bash
$ terraform import ovh_cloud_key_manager_container.my_container service_name/container_id
```
