...
# ovh_cloud_key_manager_container_consumer
...
```terraform
resource "ovh_cloud_key_manager_container_consumer" "consumer" {
...
* `id` - Consumer ID as returned by the API (URL-safe base64 of `service:resource_type:resource_id`). This is the identifier expected by the `consumer_id` argument of the `ovh_cloud_key_manager_container_consumer` data source.
...
```bash
$ terraform import ovh_cloud_key_manager_container_consumer.my_consumer service_name/container_id/service/resource_type/resource_id
```
