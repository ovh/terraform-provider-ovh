...
The provider supports named configuration profiles, similar to the `ovhcloud-cli` tool. This allows you to store multiple sets of credentials in a single `.ovh.conf` file and select which one to use via the `profile` argument (or the `OVH_PROFILE` environment variable).

Profile sections use the format `[profile:<name>]` in the `.ovh.conf` file:
...
[profile:work]
endpoint=ovh-ca
application_key=work_app_key
application_secret=work_app_secret
consumer_key=work_consumer_key
```

To use a profile in your Terraform configuration:
...
```sh
export OVH_PROFILE=personal
```

The configuration files are looked up in the same locations as the standard `.ovh.conf` file:

1. Current working directory: `./ovh.conf`
2. Current user's home directory: `~/.ovh.conf`
3. System wide configuration: `/etc/ovh.conf`

The priority order for credential resolution is: HCL provider arguments > environment variables > profile values > default `.ovh.conf` sections (loaded by go-ovh).
...
* `profile` - (Optional) The name of the OVH configuration profile to use from the `.ovh.conf` file. Uses the `[profile:<name>]` section format. If omitted, the `OVH_PROFILE` environment variable is used.
