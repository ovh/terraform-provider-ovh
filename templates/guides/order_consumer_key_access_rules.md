...
3. Use a consumer key restricted to the product API path. For a Public Cloud project, allow both `/cloud/project/<project_id>/*` and `/publicCloud/project/<project_id>/*`: many resources of the project, such as `ovh_cloud_instance` or `ovh_cloud_loadbalancer`, use the `/v2/publicCloud/project/{projectId}/...` routes.
